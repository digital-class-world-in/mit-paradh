...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log("Creating user with email:", verifiedEmail);
      const userCredential = await createUserWithEmailAndPassword(studentAuth, verifiedEmail, formData.password);
      ...
      router.push(`/login/student?registered_email=${encodeURIComponent(verifiedEmail)}`);
    } catch (error: any) {
        ...
        <div className="w-full max-w-[500px] bg-white rounded-xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
          ...
          <div className="p-8 space-y-8">
            <div className="space-y-4">
              <p className="text-sm font-bold text-slate-700">Have you registered for Paramedical Admission Previously?</p>
      ...
      <main className="max-w-6xl mx-auto w-full py-12 px-4">
        <div className="bg-white shadow-2xl rounded-3xl overflow-hidden border border-white">
          ...
          <form onSubmit={handleSubmit} className="p-10 space-y-12 bg-white">
            ...
            <div className="space-y-8 pt-8 border-t border-slate-200">
               <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-end">
                  <div className="space-y-2">
                    <label className="text-[13px] font-black text-black capitalize tracking-tight pl-2 italic">Verify Email</label>
                    <div className="flex items-center gap-4">
                      <div className="flex-1 relative">
                        <input 
                          ...
                          placeholder={verifiedEmail || "Email ID will appear here after verification"} 
                          className={`w-full border-b-2 bg-[#f0f4f8] p-4 text-sm font-bold outline-none rounded-t-xl transition-all ${emailError ? 'border-red-500 text-red-600' : verifiedEmail ? 'border-emerald-500 text-emerald-700' : 'border-slate-200 text-slate-700'}`} 
                          value={verifiedEmail}
                          readOnly
                        />
                        {emailError && (
                        ...
                        {verifiedEmail && (
                          <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-2 text-emerald-600 animate-in fade-in slide-in-from-right-2">
                            <CheckCircle size={16} />
                            <span className="text-[13px] font-black capitalize tracking-tight">Verified</span>
                          </div>
                        )}
                      </div>
                      {!verifiedEmail && (
                        <div className="pe_verify_email" data-client-id="11824745671840079560">
                           <Script src="https://www.phone.email/verify_email_v1.js" async strategy="afterInteractive" />
                        </div>
                      )}
                    </div>
                  </div>
          ...
          <footer className="bg-slate-50 px-10 py-6 text-center border-t border-slate-200">
             <p className="text-[9px] font-bold text-slate-300 capitalize tracking-[0.6em] italic">Official NSSC Registration Protocol v4.0</p>
          </footer>
        </div>
      </main>
    </div>
