...
const StatCard = ({ icon: Icon, label, value, color }: any) => (
  <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm flex items-center justify-between group hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
      ...
      if (user) {
        try {
          ...
          const unsubStudents = onValue(studentsRef, (snap) => {
            if (!isMounted) return;
            if (snap.exists()) {
              const sortedStudents = Object.entries(snap.val())
          ...
          <div className="space-y-8 animate-in fade-in duration-700">
            ...
            <div className="relative bg-[#5D5fb1] rounded-[3.5rem] p-12 overflow-hidden shadow-2xl border-b-8 border-[#00a5a5]">
              ...
              <div className="relative z-10 flex flex-col md:flex-row items-center gap-10">
                 <div className="w-40 h-40 rounded-[2.5rem] border-4 border-white/20 shadow-2xl overflow-hidden bg-white/10 flex items-center justify-center p-1">
                    <div className="w-full h-full rounded-[2.2rem] overflow-hidden bg-white">
                      {collegeData?.headPhoto ? (
                        <img src={collegeData.headPhoto} alt="Head" className="w-full h-full object-cover" />
                      ) : (
                        <Building2 size={48} className="text-[#5D5fb1]" />
                      )}
                    </div>
                 </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <StatCard icon={Users} label="Total Staff" value={staffList.length.toString()} color="text-blue-500" />
              <StatCard icon={GraduationCap} label="Total Students" value={studentList.length.toLocaleString()} color="text-purple-500" />
              <StatCard icon={BookOpen} label="Active Courses" value={courses.length.toString()} color="text-emerald-500" />
              <StatCard icon={Clock} label="Pending Admission" value={inquiries.filter((i: any) => i.status === 'Pending').length.toString().padStart(2, '0')} color="text-amber-500" />
            </div>
