...
const DashboardHome = ({ userData, userApplications, stepPercentages, feeDue, hasActiveAdmission, setIsOtherCourseMode, setIsCourseModalOpen, handleTabChange }: any) => (
  <div className="space-y-6 animate-in fade-in duration-500">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
       ...
       <Widget 
         ...
         value={userData?.course || 'Not Selected'} 
         trend="Active" 
         color="bg-purple-600" 
       />
    </div>
    ...
    <div className="flex flex-col lg:flex-row gap-6">
      ...
      <div className="lg:w-3/4 grid grid-cols-1 md:grid-cols-2 gap-3 auto-rows-min">
          {[
            ...
            { l: 'Course', v: userData?.course || 'Not Selected' },
            { l: 'Form number', v: 'F-2026/00452' },
            { l: 'Manual reg no.', v: 'MAN-922-88' },
            { l: 'Course fees', v: feeDue },
          ].map((item, i) => (
...
const ApplicationManager = ({ 
  ...
  userApplications, userData, handleTabChange
}: any) => {
  const isLocked = userData?.profile?.profileLocked || userData?.profile?.status === 'Confirmed' || userData?.profile?.status === 'Accepted';
        ...
        <div className="bg-white border-[0.5px] border-black rounded-md shadow-sm overflow-hidden text-[#343a40]">
          <div className="bg-[#002147] text-white py-4 px-8 font-black text-sm tracking-widest flex items-center justify-between uppercase">
            ...
            <div className="flex items-center gap-2 px-3 py-1 bg-white/10 rounded-full border border-white/20 text-[10px]">
              <ShieldCheck size={12} className="text-[#00a5a5]" /> PROFILE SECURED
            </div>
          </div>
            ...
            <table className="w-full text-left border-collapse">
               <tbody>
                  <tr className="border-b-[0.5px] border-black">
                    ...
                    <td className="px-8 py-5 border-r-[0.5px] border-black text-sm font-bold uppercase">{userData?.profile?.regNo || userData?.regNo || 'N/A'}</td>
                    ...
                    <td className="px-8 py-5 text-sm font-bold capitalize">{`${userData?.profile?.firstName || ''} ${userData?.profile?.lastName || ''}`.trim() || 'N/A'}</td>
                  </tr>
                  <tr className="border-b-[0.5px] border-black">
                    <td className="px-8 py-5 border-r-[0.5px] border-black bg-slate-50 font-black text-[11px] text-[#00a5a5] uppercase">Gender</td>
                    <td className="px-8 py-5 border-r-[0.5px] border-black text-sm font-bold uppercase">{userData?.profile?.gender || 'N/A'}</td>
                    ...
                    <td className="px-8 py-5 text-sm font-bold uppercase">{userData?.profile?.dateOfBirth || 'N/A'}</td>
                  </tr>
                  <tr className="border-b-[0.5px] border-black">
                    ...
                    <td className="px-8 py-5 border-r-[0.5px] border-black text-sm font-bold uppercase">{userData?.profile?.mobileNumber || userData?.phone || 'N/A'}</td>
                    <td className="px-8 py-5 border-r-[0.5px] border-black bg-slate-50 font-black text-[11px] text-[#00a5a5] uppercase">Email ID</td>
                    <td className="px-8 py-5 text-sm font-bold">{userData?.email || 'N/A'}</td>
                  </tr>
                  <tr className="border-b-[0.5px] border-black">
                    ...
                    <td className="px-8 py-5 border-r-[0.5px] border-black text-sm font-bold uppercase">{userData?.profile?.aadhaarNo || 'N/A'}</td>
                    <td className="px-8 py-5 border-r-[0.5px] border-black bg-slate-50 font-black text-[11px] text-[#00a5a5] uppercase">Status</td>
                    <td className="px-8 py-5 text-sm font-black text-emerald-600 uppercase">LOCKED / VERIFIED</td>
                  </tr>
                  <tr className="border-b-[0.5px] border-black">
                    <td className="px-8 py-5 border-r-[0.5px] border-black bg-slate-50 font-black text-[11px] text-[#00a5a5] uppercase">Current College</td>
                    <td className="px-8 py-5 border-r-[0.5px] border-black text-sm font-bold uppercase">{userApplications[0]?.collegeName || 'N/A'}</td>
                    <td className="px-8 py-5 border-r-[0.5px] border-black bg-slate-50 font-black text-[11px] text-[#00a5a5] uppercase">Current Course</td>
                    <td className="px-8 py-5 text-sm font-bold uppercase">{userApplications[0]?.courseName || 'N/A'}</td>
                  </tr>
               </tbody>
            </table>
          </div>
...
const FeePaymentPortal = ({ acceptedApps, totalFees, totalPaid, balanceDue, userPayments, setSelectedAppForPayment, setPaymentForm, setIsPaymentModalOpen }: any) => (
  <div className="animate-in slide-in-from-bottom-8 duration-500 space-y-8">
     ...
     {acceptedApps.some((app: any) => app.status === 'Confirmed') && (
        <div className="bg-white border-4 border-black rounded-[2rem] p-8 shadow-xl animate-in zoom-in-95 duration-500 overflow-hidden relative">
           <div className="absolute top-0 right-0 w-32 h-32 bg-[#00a5a5]/10 rounded-full -mr-16 -mt-16" />
           <div className="flex flex-col md:flex-row items-center justify-between gap-8 relative z-10">
              <div className="flex items-center gap-6">
                 ...
                 <div>
                    <h4 className="text-[11px] font-black text-emerald-600 uppercase tracking-widest mb-1">Confirmed Admission Fee</h4>
                    <h3 className="text-2xl font-black text-[#002147] tracking-tighter capitalize">
                       {acceptedApps.find((app: any) => app.status === 'Confirmed')?.courseName}
                    </h3>
                    <p className="text-xs font-medium text-slate-400 capitalize mt-1">Official Institutional Fee Registry — Verified</p>
                 </div>
              </div>
              <div className="flex items-center gap-12 bg-slate-50 px-10 py-6 rounded-3xl border-2 border-black/5">
                 <div className="text-center">
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Total Course Fee</p>
                    <p className="text-3xl font-black text-[#002147] tracking-tighter">₹{parseFloat(acceptedApps.find((app: any) => app.status === 'Confirmed')?.fees || '0').toLocaleString()}</p>
                 </div>
                 <div className="w-[1px] h-12 bg-slate-200" />
                 <div className="text-center">
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Balance Remaining</p>
                    <p className="text-3xl font-black text-rose-500 tracking-tighter">₹{balanceDue.toLocaleString()}</p>
                 </div>
              </div>
           </div>
        </div>
           ...
           <table className="w-full text-left border-collapse border-[0.5px] border-black">
              <thead>
                 <tr className="text-[14px] font-normal text-[#00a5a5] border-b-[0.5px] border-black bg-slate-50">
                    ...
                    <th className="px-6 py-4 border-r-[0.5px] border-black text-center">Payment Status</th>
                 ...
                 {acceptedApps.map((app: any, idx: number) => {
                   ...
                   return (
                     <tr key={idx} className="hover:bg-slate-50 transition-colors border-b-[0.5px] border-black">
                        ...
                        <td className="px-6 py-6 border-r-[0.5px] border-black text-center">
                           <div className="flex flex-col gap-2">
                              {userPayments.filter((pay: any) => pay.applicationId === app.applicationId).map((pay: any, pIdx: number) => (
                                 <div key={pIdx} className={cn("px-3 py-1 rounded-full text-[9px] font-black uppercase border shadow-sm", pay.status === 'Accepted' ? "bg-emerald-50 text-emerald-600 border-emerald-100" : "bg-amber-50 text-amber-600 border-amber-100")}>₹{parseFloat(pay.amount || '0').toLocaleString()} - {pay.status || 'Pending'}</div>
                              ))}
                           </div>
                        </td>
...
const PrintApplicationRegistry = ({ userApplications, userData }: any) => {
  return (
          ...
          <tbody className="border-b-[0.5px] border-black text-[15px] font-medium text-black">
            {userApplications.filter((app: any) => app.profileLocked === true).map((app: any, i: number) => (
              <tr key={app.id} className="hover:bg-slate-50 transition-colors border-b-[0.5px] border-black whitespace-nowrap">
                ...
                <td className="px-4 py-6 text-center">
                   <div className="flex items-center justify-center gap-3">
                      <button onClick={() => window.print()} className="w-10 h-10 rounded-xl bg-emerald-50 text-emerald-600 border border-emerald-200 flex items-center justify-center hover:bg-emerald-600 hover:text-white transition-all shadow-sm active:scale-90" title="Preview Application"><Eye size={18} /></button>
                      <button onClick={() => {
                        const link = document.createElement('a');
                        link.href = '#'; 
                        link.download = `Application_${app.applicationId}.pdf`;
                        link.click();
                      }} className="w-10 h-10 rounded-xl bg-[#002147] text-white flex items-center justify-center hover:bg-black transition-all shadow-md active:scale-90" title="Download Application"><Download size={18} /></button>
                   </div>
                </td>
              </tr>
            ))}
