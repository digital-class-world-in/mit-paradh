...
export const Navbar = () => {
  ...
  return (
    <>
      <nav className="bg-[#003366] px-4 sm:px-6 lg:px-10 h-14 flex items-center justify-between shadow-md sticky top-0 z-50 select-none">
        
        {/* Mobile Hamburger Button */}
        <button 
          className="lg:hidden text-white p-2 rounded-lg hover:text-amber-400 hover:bg-white/10 transition-colors focus:outline-none focus:ring-2 focus:ring-amber-400/50"
        ...
        {/* Mobile quick brand or title for small screens */}
        <div className="lg:hidden flex items-center">
          <Link 
            href="/" 
            className="text-white font-extrabold text-sm sm:text-base tracking-tight truncate max-w-[200px] sm:max-w-[320px]"
          ...
          {mainNav.map((item, idx) => (
            ...
            >
              <Link 
                ...
                className="flex items-center gap-1.5 xl:gap-2 px-3.5 xl:px-6 h-full text-white/80 hover:text-white hover:bg-white/10 transition-all font-black text-[15px] xl:text-[17px] capitalize tracking-tight border-r border-white/10 cursor-pointer"
              >
              ...
              {item.hasDropdown && (
                <div 
                  className={`absolute left-0 top-full pt-2 w-[380px] xl:w-[400px] transition-all duration-300 origin-top-left z-[999] ${
        ...
        <div className="flex items-center gap-2 sm:gap-4 relative shrink-0">
          ...
          <div className="relative group">
            <Link 
              ...
              className="bg-amber-400 hover:bg-amber-500 text-[#003366] px-3.5 sm:px-5 lg:px-6 xl:px-8 py-2 sm:py-2.5 rounded-md font-black text-xs sm:text-sm lg:text-[15px] xl:text-[17px] capitalize tracking-tight transition-all flex items-center gap-1.5 sm:gap-2 shadow-lg active:scale-95"
            >
              <LogIn size={15} /> 
              <span>Login</span>
              <ChevronDown size={13} className={`hidden sm:inline transition-transform duration-300 ${isLoginOpen ? 'rotate-180' : ''}`} />
            </Link>
            ...
            <div 
              onMouseEnter={() => setIsLoginOpen(true)}
              onMouseLeave={() => setIsLoginOpen(false)}
              className={`hidden sm:block absolute right-0 top-full pt-2 w-64 transition-all duration-300 origin-top-right ${isLoginOpen ? 'opacity-100 scale-100 visible' : 'opacity-0 scale-95 invisible'}`}
            >
              <div className="bg-[#002244] border border-white/10 rounded-lg shadow-2xl overflow-hidden backdrop-blur-xl">
                <div className="bg-white/5 p-3 border-b border-white/5 flex items-center justify-between">
                  ...
                  <Link href="/login" className="text-[9px] font-bold text-amber-400 hover:underline">View All</Link>
                </div>
        ...
        {isMobileMenuOpen && (
          <div className="lg:hidden absolute top-full left-0 right-0 bg-[#002244] border-t border-white/10 shadow-2xl flex flex-col py-2 z-[999] max-h-[82vh] overflow-y-auto overscroll-contain">
            {mainNav.map((item, idx) => (
              <div key={idx} className="flex flex-col">
                <Link 
                  ...
                  className="flex items-center justify-between px-6 py-3.5 min-h-[48px] text-white/90 hover:text-white hover:bg-white/10 transition-all font-bold text-[15px] border-b border-white/5 active:bg-white/15"
                >
                  <div className="flex items-center gap-3">
                    <item.icon size={18} className="opacity-60 text-amber-400" />
                ...
                {item.hasDropdown && isCoursesOpen && (
                  <div className="bg-[#001428] flex flex-col px-4 py-3 gap-2.5 border-b border-white/5">
                    {/* Mobile Search Bar */}
                      ...
                      {allCourses.length > 0 ? (() => {
                        ...
                        return mobileFiltered.map((course) => (
                          <Link
                            ...
                            className="flex items-center gap-3 py-3 px-2 border-b border-white/5 last:border-0 group hover:bg-white/5 rounded-lg transition-colors"
                          >
                            <div className="w-7 h-7 rounded-lg bg-amber-400/10 flex items-center justify-center text-amber-400 shrink-0">
                              <GraduationCap size={14} />
                            </div>
                            <span className="text-white/90 text-[13px] font-semibold group-hover:text-amber-400 leading-snug transition-colors truncate">
                              {course.course_name || course.name}
                            </span>
                          </Link>
                        ));
            ...
            {/* Mobile Login Portals Section */}
            <div className="mt-2 pt-3 border-t border-white/10 px-4 pb-4">
              <p className="text-[10px] font-black text-amber-400 uppercase tracking-widest px-2 mb-2">Portal Logins</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5">
                {portals.map((portal, idx) => (
                  <Link
                    ...
                    href={portal.href}
                    onClick={() => setIsMobileMenuOpen(false)}
                    className="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-white/10 text-white/90 text-[13px] font-bold transition-all min-h-[44px]"
                  >
                ...
                <Link
                  href="/login"
                  onClick={() => setIsMobileMenuOpen(false)}
                  className="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-amber-400/10 text-amber-300 text-[13px] font-bold mt-1 min-h-[44px]"
                >
                  <LogIn size={16} />
                  <span>All Login Portals</span>
                </Link>
                <Link
                  href="/register"
                  onClick={() => setIsMobileMenuOpen(false)}
                  className="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-emerald-500/10 text-emerald-300 text-[13px] font-bold mt-1 min-h-[44px]"
                >
                  <User size={16} />
