...
export default function LiveFooter() {
  ...
  return (
    <footer className="bg-[#020817] text-white py-16 px-6 lg:px-16 border-t border-white/5 font-sans relative overflow-hidden">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-10 relative z-10">
        <div className="md:col-span-3 space-y-6">
          <div>
            ...
            <h3 className="text-sm font-black tracking-tight text-white uppercase leading-snug">{data.sansthaName}</h3>
            <p className="text-xs font-bold text-amber-400 mt-1 leading-relaxed">{data.instituteName}</p>
          </div>
          <div className="flex gap-3">
            {data.instagramUrl && (
              <a href={data.instagramUrl} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-xl bg-white/5 hover:bg-gradient-to-tr hover:from-yellow-600 hover:to-purple-600 flex items-center justify-center transition-all text-white border border-white/10 hover:border-transparent">
            ...
            {data.facebookUrl && (
              <a href={data.facebookUrl} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-xl bg-white/5 hover:bg-blue-600 flex items-center justify-center transition-all text-white border border-white/10 hover:border-transparent">
                <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24"><path d="M9 8h-3v4h3v12h5v-12h3.642l.358-4h-4v-1.667c0-.955.192-1.333 1.115-1.333h2.885v-5h-3.808c-3.596 0-5.192 1.583-5.192 4.615v3.385z"/></svg>
              </a>
            )}
            {data.youtubeUrl && (
              <a href={data.youtubeUrl} target="_blank" rel="noopener noreferrer" className="w-9 h-9 rounded-xl bg-white/5 hover:bg-red-600 flex items-center justify-center transition-all text-white border border-white/10 hover:border-transparent">
        ...
        <div className="md:col-span-2">
          <h4 className="font-bold text-white mb-5 capitalize text-xs tracking-tight border-b border-white/10 pb-3">{data.aboutHeading}</h4>
          <ul className="space-y-3">
            {data.aboutLinks.map((link: any, idx: number) => (
              <li key={idx}><Link href={link.url} className="flex items-center gap-2 text-blue-300 hover:text-white transition-colors text-sm"><ChevronRight size={14} /> {link.label}</Link></li>
            ))}
          </ul>
        </div>

        <div className="md:col-span-2">
          <h4 className="font-bold text-white mb-5 capitalize text-xs tracking-tight border-b border-white/10 pb-3">{data.supportHeading}</h4>
          <ul className="space-y-3">
        ...
        <div className="md:col-span-2">
          <h4 className="font-bold text-white mb-5 capitalize text-xs tracking-tight border-b border-white/10 pb-3">{data.policiesHeading}</h4>
          <ul className="space-y-3">
        ...
        <div className="md:col-span-3">
          <h4 className="font-bold text-white mb-5 capitalize text-xs tracking-tight border-b border-white/10 pb-3">Contact Info</h4>
            ...
            {(data.branches || []).map((branch: any, idx: number) => {
              ...
              return (
                <div key={idx} className="space-y-4 border-b border-white/5 pb-4 last:border-0 last:pb-0">
                  {showLabels && (
                    <p className="text-[10px] font-black text-amber-400 uppercase tracking-widest mb-2">{labelText}</p>
                  )}
                  {branch.address?.trim() && (
                    <div className="flex gap-3 items-start">
                      ...
                      <a 
                        ...
                        className="text-sm font-semibold leading-relaxed whitespace-pre-line text-white hover:text-amber-400 transition-colors"
                      >
                        {branch.address}
                      </a>
                    </div>
      ...
      <div className="border-t border-white/5 mt-12 pt-6 px-6 lg:px-16 flex flex-col items-center justify-center text-center gap-2">
        {data.copyrightText?.trim() && (
          <p className="text-white/35 font-semibold text-[10px] sm:text-xs md:text-sm tracking-wide">
            {data.copyrightText}
          </p>
        )}
      </div>
