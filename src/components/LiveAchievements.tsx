      ...
      <div className="px-6 lg:px-12">
        ...
        <div className="relative group">
          ...
          <div 
            ...
            className="flex gap-5 overflow-x-auto no-scrollbar pb-4 snap-x"
          >
            {achievements.map((ach, i) => (
              <div key={i} className="min-w-[220px] md:min-w-[260px] snap-center bg-white rounded-lg p-8 text-center border border-slate-200 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300 shrink-0">
                <div className="w-16 h-16 bg-amber-50 rounded-full flex items-center justify-center text-amber-600 mx-auto mb-6">
                  <Trophy size={32} />
                </div>
                <h3 className="text-3xl md:text-4xl font-black text-slate-800 tracking-tighter mb-2">{ach.metric}</h3>
                <p className="text-sm font-bold text-[#003366] capitalize tracking-tight mb-2">{ach.title}</p>
                {ach.description && <p className="text-xs font-semibold text-slate-500">{ach.description}</p>}
              </div>
            ))}
          </div>
