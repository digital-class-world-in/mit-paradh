          ...
          {!isLoggedIn ? (
            <div className="relative" ref={dropdownRef}>
              <button 
                ...
                className="bg-institutional-gold text-[#002147] px-8 py-3 rounded-xl text-[15px] font-black tracking-tight hover:bg-white transition-all shadow-lg shadow-yellow-900/20 active:scale-95 flex items-center gap-2"
              >
                Portal access <ChevronDown size={14} className={`transition-transform duration-300 ${showDropdown ? 'rotate-180' : ''}`} />
              </button>
                ...
                <div className="absolute right-0 mt-3 w-64 bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden py-2 animate-in fade-in slide-in-from-top-4 duration-300 z-[110]">
                   {[
                     ...
                     >
                        ...
                        <span className="text-[13px] font-black text-black tracking-tight">{item.label}</span>
                     </Link>
                   ))}
                </div>
              )}
            ...
            <div className="flex items-center gap-4">
              <Link href="/dashboard" className="flex items-center gap-2 text-[13px] font-black text-black hover:text-black tracking-tight transition-colors bg-white/5 px-5 py-3 rounded-xl border border-white/10">
                <LayoutDashboard size={14} className="text-institutional-gold" /> Dashboard
              </Link>
              <button 
                onClick={handleLogout}
                className="bg-red-600 text-black px-8 py-3 rounded-xl text-[13px] font-black tracking-tight shadow-lg shadow-red-900/20 hover:bg-red-700 transition-all flex items-center gap-2 active:scale-95"
              >
