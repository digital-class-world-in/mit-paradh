...
import { LogOut, ChevronDown, Trash2, Globe, CreditCard } from 'lucide-react';
...
export default function StaffHeader({ activeTab, setActiveTab, onLogout, staffName, collegeLogo, collegeName, permissions }: StaffHeaderProps) {
  ...
  const menuItems = [
    ...
    { 
      label: 'Front office', 
      id: 90,
      subItems: [{ label: 'Admission inquiry', id: 91 }]
    },
      ...
      <div className="max-w-full mx-auto px-6 h-24 flex items-center justify-between">
        <div className="flex items-center gap-4 shrink-0 cursor-pointer" onClick={() => setActiveTab(1)}>
          ...
          <div className="hidden lg:block max-w-[200px]">
            ...
            <p className="text-[13px] capitalize font-bold tracking-tight text-black">Staff Portal</p>
          </div>
        </div>
              ...
              <button 
                ...
                className={cn(
                  ...
                  (activeTab === group.id || group.subItems?.some(i => i.id === activeTab))
                    ? (group.label === 'Front office' ? "text-black bg-slate-50 font-normal" : "text-[#5D5fb1] bg-slate-50")
                  ...
                  {group.subItems.map((item, iIdx) => (
                    <button 
                      ...
                      className={cn(
                        ...
                        (activeTab === item.id || (item as any).url === pathname) ? "text-[#5D5fb1] font-normal bg-slate-50" : "text-black hover:text-[#5D5fb1] hover:bg-slate-50"
                      )}
                    >
                      {item.label}
                    </button>
        ...
        <div className="flex items-center gap-4 shrink-0">
          <div className="hidden lg:block text-right">
            <p className="text-[16px] font-semibold text-slate-800 tracking-tight mt-1 capitalize">{staffName || 'Staff'}</p>
          </div>
          <button 
            onClick={onLogout}
