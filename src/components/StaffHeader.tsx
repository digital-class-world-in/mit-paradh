...
export default function StaffHeader({ activeTab, setActiveTab, onLogout, staffName, collegeLogo, collegeName, permissions }: StaffHeaderProps) {
  ...
  const menuItems = [
    ...
    { 
      ...
      icon: GraduationCap,
      subItems: [
        { label: 'Pending Admission', url: '/staff/dashboard/pending-admissions' },
        { label: 'Confirm Admission', url: '/staff/dashboard/confirm-admissions' }, 
        { label: 'Cancel Admission', id: 1023 }, 
        { label: 'Approved Admission', url: '/staff/dashboard/approved-admissions' },
        { label: 'Admission List', id: 2022 },
        { label: 'Student Admission', id: 25 },
        { label: 'Student Registration', id: 19 },
        { label: 'Admission Reports', id: 24 },
  ...
  // Filter items for mobile drawer live search
  ...
  const handleItemNavigation = (item: any, parentGroup?: any) => {
    if (item.isExternal) {
      window.open('/', '_blank');
      setOpenDropdown(null);
      setIsMobileMenuOpen(false);
      ...
      if (parentGroup && parentGroup.id) {
        setActiveTab(parentGroup.id, item.id, parentGroup.label, item.label);
      } else {
    ...
    >
      <div className="max-w-full mx-auto px-3 sm:px-6 h-16 sm:h-20 flex items-center justify-between gap-2">
        ...
        >
          ...
          <div className="max-w-[130px] sm:max-w-[180px] md:max-w-[240px]">
            <h1 className="text-xs sm:text-sm md:text-base font-bold text-[#00a5a5] truncate leading-tight">
              {collegeName || 'MIT PARADH'}
            </h1>
            <p className="text-[10px] sm:text-xs capitalize font-semibold tracking-tight text-slate-500 truncate">
              Staff Portal
            </p>
          </div>
        </div>

        {/* Mobile & Tablet Action Bar (Website, 1-tap Logout, Hamburger) */}
                  ...
                  <div className="absolute top-full left-0 mt-1 min-w-[210px] bg-white text-black rounded-xl shadow-2xl py-1.5 border border-slate-200 z-[100] animate-in fade-in slide-in-from-top-2 duration-150 max-h-[70vh] overflow-y-auto">
                    {group.subItems.map((item, iIdx) => {
                      ...
                      return (
                        <button 
                          key={iIdx} 
                          onClick={() => handleItemNavigation(item, group)}
                          className={cn(
        ...
        <div className="hidden xl:flex items-center gap-2.5 shrink-0">
          <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-xl">
            <div className="w-6 h-6 rounded-full bg-[#00a5a5]/15 text-[#00a5a5] flex items-center justify-center font-bold text-xs">
              {(staffName || 'S')[0].toUpperCase()}
            </div>
            <span className="text-xs font-semibold text-slate-700 capitalize truncate max-w-[130px]">
          ...
          >
            ...
            <div className="p-4 flex items-center justify-between border-b border-slate-200 bg-slate-50/60">
              <div className="flex items-center gap-3">
                 <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center shadow-md border-2 border-slate-200 shrink-0 overflow-hidden">
                    {collegeLogo ? (
                      <img src={collegeLogo} alt="Logo" className="w-full h-full object-contain p-0.5" />
                    ) : (
                      <img src="https://ik.imagekit.io/gnzjd77mb/WhatsApp%20Image%202026-04-23%20at%2014.44.57.jpeg" alt="Logo" className="w-full h-full object-contain" />
                 ...
                 <div className="max-w-[170px]">
                    ...
                    <p className="text-[10px] text-slate-500 uppercase tracking-widest font-semibold truncate">
                      {staffName ? `${staffName} â€¢ Staff` : 'Staff Portal'}
                    </p>
                 </div>
              </div>
              <button 
            ...
            <div className="p-3 space-y-1 flex-1 overflow-y-auto">
               {searchedDrawerItems.length > 0 ? (
                           ...
                           {group.subItems.map((item: any, iIdx: number) => {
                             ...
                             return (
                               <button
                                 ...
                                 onClick={() => handleItemNavigation(item, group)}
                                 className={cn(
                                   "w-full text-left px-3 py-2 rounded-lg text-xs font-medium transition-all flex items-center justify-between min-h-[38px]",
                                   isItemActive 
                                     ? "text-[#5D5fb1] font-bold bg-white shadow-sm" 
                 ...
                 <div className="text-center py-8 text-slate-400">
                   <p className="text-xs font-medium">No matching modules found</p>
                   <button 
                     onClick={() => setDrawerSearch('')} 
                     className="mt-2 text-[11px] text-[#00a5a5] hover:underline font-semibold"
                   >
                     Clear search
                   </button>
                 </div>
            ...
            <div className="p-4 border-t border-slate-200 bg-slate-50/50 space-y-2">
              <Link
                ...
                target="_blank"
                onClick={() => setIsMobileMenuOpen(false)}
                className="w-full flex items-center justify-center gap-2 p-2.5 rounded-xl border border-slate-200 bg-white text-slate-700 hover:bg-slate-50 text-xs font-bold transition-all"
              >
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 p-2.5 rounded-xl bg-red-600 hover:bg-red-700 active:scale-95 text-white text-xs font-bold transition-all shadow-sm"
              >
                <LogOut size={15} />
                <span>Logout</span>
              </button>
