...
export default function StaffHeader({ activeTab, setActiveTab, onLogout, staffName, collegeLogo, collegeName, permissions }: StaffHeaderProps) {
  ...
  const menuItems = [
    ...
    { 
      label: 'Fees', 
      id: 70,
      subItems: [{ label: 'Collect fees', id: 71 }]
    },
    { 
    ...
    { label: 'Settings', id: 5, icon: Settings },
                  ...
                  {group.subItems.map((item, iIdx) => (
                    <button 
                      ...
                      className={cn(
                        ...
                        activeTab === item.id ? "text-[#5D5fb1] font-normal bg-slate-50" : "text-black hover:text-[#5D5fb1] hover:bg-slate-50"
                      )}
                    >
                      {item.label}
                    </button>
