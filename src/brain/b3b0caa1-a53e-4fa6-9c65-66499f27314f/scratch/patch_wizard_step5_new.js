...
// 1. Add "Other" board input
const boardTarget = `<select 
...
if (content.includes(boardTarget)) {
    content = content.replace(boardTarget, boardReplacement);
    console.log('Updated Board selection with Other option');
}

// 2. Add Live Preview button in Form
const uploadTarget = `<label className="bg-white border-2 border-dashed border-orange-200 hover:border-orange-500 px-6 py-4 rounded-xl flex items-center gap-3 cursor-pointer transition-all flex-1 group">
...
// 3. Add Preview button in Table
const tableActionTarget = `<button 
...
if (content.includes(tableActionTarget)) {
    content = content.replace(tableActionTarget, tableActionReplacement);
