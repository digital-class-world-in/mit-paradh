...
// 1. Add modalPreview state
if (!content.includes('const [modalPreview, setModalPreview] =')) {
    content = content.replace('const [docPreviews, setDocPreviews] = useState<Record<number, boolean>>({});', 
        'const [modalPreview, setModalPreview] = useState<{ url: string, label: string } | null>(null);');
    console.log('Updated state from docPreviews to modalPreview');
}

// 2. Update Document Vault UI to compact "field" layout
const vaultGridPattern = /<div className="grid grid-cols-1 md:grid-cols-3 gap-6">\s+\[\s+\{ label: 'Aadhar Card Front', key: 'aadhaarFrontUrl' \},[\s\S]+?<\/div>\s+\);\s+\}\)\}\s+<\/div>/;

const newVaultUI = `<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {[
                    { label: 'Aadhar Card Front', key: 'aadhaarFrontUrl' },
                    { label: 'Aadhar Card Back', key: 'aadhaarBackUrl' },
                    { label: '10th Marksheet', key: 'ssc' },
                    { label: '12th Marksheet', key: 'hsc' },
...
if (vaultGridPattern.test(content)) {
    content = content.replace(vaultGridPattern, newVaultUI);
    console.log('Updated Document Vault to compact field layout');
}

// 3. Add Preview Modal at the end of the return block
const returnEnd = /<\/div>\s+<\/div>\s+\);\s+\}/;
...
if (returnEnd.test(content)) {
    content = content.replace(returnEnd, newReturnEnd);
    console.log('Added Document Preview Modal');
}
